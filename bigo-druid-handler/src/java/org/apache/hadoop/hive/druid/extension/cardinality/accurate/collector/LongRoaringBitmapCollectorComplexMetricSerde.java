...
import org.apache.hadoop.hive.druid.extension.cardinality.accurate.VariableConfig;
...
public class LongRoaringBitmapCollectorComplexMetricSerde extends ComplexMetricSerde
{
  ...
  public ComplexMetricExtractor getExtractor()
  {
    return new ComplexMetricExtractor()
    {
      ...
      public LongRoaringBitmapCollector extractValue(InputRow inputRow, String metricName)
      {
        ...
        LongRoaringBitmapCollector collector =
                (LongRoaringBitmapCollector) longBitmapCollectorFactory.makeEmptyCollector();
        ...
        if (VariableConfig.getOpenOneId(metricName).equals("false")) {
          ...
          Long resuiltId;
          try {
            String key = objectStr + VariableConfig.getNameSpace(metricName);
            resuiltId = Long.valueOf(cache.get(key, new Callable<String>() {
              @Override
              public String call() {
                return getId(VariableConfig.getNameSpace(metricName), key);
              }
            }));
          } catch (Exception e) {
  ...
  private String getId(String namespace, String key) {
    ...
    JsonNode result = doGet(VariableConfig.getOneIdUrl(), params, "UTF-8");
